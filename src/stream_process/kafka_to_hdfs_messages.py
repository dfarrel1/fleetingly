...
class Consumer(object):
    ...
    folders, /user/fleetingly/history/ and /user/fleetingly/cached.
    ...
    def __init__(self, addr, group, topic):
        ...
        self.temp_file = None
        self.hadoop_path = "/user/fleetingly/history"
        self.cached_path = "/user/fleetingly/cached"
        self.topic = topic
        self.group = group
        self.block_cnt = 0
    ...
    def consume_topic(self, output_dir):
        ...
        self.temp_file_path = "%s/kafka_%s_%s_%s.txt" % (output_dir,
                                                         self.topic,
                                                         self.group,
                                                         timestamp)
    ...
    def flush_to_hdfs(self, output_dir):
        ...
        hdfs://user/fleetingly/history and
        hdfs://user/fleetingly/cached
        ...
        hadoop_fullpath = "%s/%s_%s_%s.txt" % (self.hadoop_path, self.group,
                                               self.topic, timestamp)
        cached_fullpath = "%s/%s_%s_%s.txt" % (self.cached_path, self.group,
                                               self.topic, timestamp)
...
if __name__ == '__main__':
    ...
    print "\nConsuming messages..."
    cons = Consumer(addr="localhost:9092", group="hdfs", topic="stream_basic")
